...
AOP的使用，我们需要在切面类上标注@EnableAspectJAutoProxy注解，那我们分析aop原理，就从这个注解开始分析。

@EnableAspectJAutoProxy是什么？
在@EnableAspectJAutoProxy里，它会调用@Import(AspectJAutoProxyRegistrar.class)：给容器中导入AspectJAutoProxyRegistrar，然后利用AspectJAutoProxyRegistrar自定义给容器中注册bean，注册的bean为AnnotationAwareAspectJAutoProxyCreator；所以接下来就着重分析AnnotationAwareAspectJAutoProxyCreator。
...
上面是AnnotationAwareAspectJAutoProxyCreator的继承结构，最顶端是实现了SmartInstantiationAwareBeanPostProcessor和BeanFactoryAware两个接口。
 我们首先看一下最顶级的父类AbstractAutoProxyCreator；
 AbstractAutoProxyCreator有后置处理器的逻辑，会调用initBeanFactory方法来初始化bean工厂。
...
在调用完refresh（）方法后，就调用finishBeanFactoryInitialization方法，完成BeanFactory初始化工作；创建剩下的单实例bean
...
2.1）、获取当前bean的所有增强器（通知方法）  Object[]  specificInterceptors
	...
	2、获取到能在bean使用的增强器。(通过在切面类中标注的注解来判断，看有没有标注aop的注解)
...
2.3.4）、给容器中返回当前组件使用cglib增强了的代理对象；

2.3.5）、以后容器中获取到的就是这个组件的代理对象，执行目标方法的时候，代理对象就会执行通知方法的流程；
3）、目标方法执行	；
...
3.2）、根据ProxyFactory对象获取将要执行的目标方法拦截器链；
	...
	在getInterceptorsAndDynamicInterceptionAdvice中，会返回一个list:interceptorList
...
3.2.2）、遍历所有的增强器，将其转为Interceptor；
	...
	是通过registry.getInterceptors(advisor);来实现
...
### ![img](https://img-blog.csdnimg.cn/2018112314211867.jpg?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3B6cTkxNTk4MTA0OA==,size_16,color_FFFFFF,t_70)

上图的执行顺序为：从上到下，到了MethodBeforeAdviceInterceptor的时候，执行前置通知，然后执行目标方法。然后执行后置通知，接下来就依次向上执行，并执行返回通知，异常通知。
